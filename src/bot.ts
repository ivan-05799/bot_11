...
async function getOrCreateUserFunnel(chatId: number) {
  ...
  try {
    ...
    // Пытаемся получить существующую воронку
    ...
    // ВАЖНО: НЕ БРОСАЕМ ОШИБКУ, а возвращаем null или обрабатываем
    console.error(`❌ Ошибка при создании воронки для ${chatId}:`, error.message);
    
    // Если ошибка внешнего ключа - администратора нет в user_access
    if (error.code === '23503') {
      console.log(`⚠️ Администратор ${chatId} не найден в user_access`);
      return null; // или можно создать временную запись без внешнего ключа
    }
...
bot.hears('🎯 Выбрать вертикаль', async (ctx) => {
  ...
  // Создаем или получаем воронку
  const funnel = await getOrCreateUserFunnel(chatId);
  
  // ВАЖНО: Проверяем, удалось ли создать/получить воронку
  if (!funnel) {
    // Если админ - сообщаем об ошибке внешнего ключа
    if (isAdmin(chatId)) {
      await ctx.reply(
        `*⚠️ Ошибка доступа администратора*\n\n` +
        `Ваш аккаунт не зарегистрирован в системе доступа\\.\n` +
        `Пожалуйста\\, обратитесь к техническому администратору\\.\n\n` +
        `_Или используйте команду /admin для управления системой_`,
        { parse_mode: 'MarkdownV2' }
      );
    } else {
      await ctx.reply(
        `*⚠️ Ошибка системы*\n\n` +
        `Не удалось инициализировать процесс\\.\n` +
